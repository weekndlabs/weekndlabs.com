...
test('the focus pair is a pair, and the cards it fills need tags', () => {
  // The section renders a product as a card when it carries `focus` and as a row
  // otherwise, so a third card would break the two-column grid and a focus
  // product with no tags renders an empty tag strip.
  const focus = PRODUCTS.filter((product) => product.focus);
